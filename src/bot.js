...
function getBalance(chatId) {
  ...
  return balances[String(chatId)] || 0;
}
...
function setBalance(chatId, amount) {
  const balances = loadBalances();
  balances[String(chatId)] = Math.round(amount * 100) / 100;
  saveBalances(balances);
  return balances[String(chatId)];
}
...
bot.on('message', async (msg) => {
  ...
  if (adminStates[chatId] && adminStates[chatId].state === 'ADMIN_EDIT_BALANCE_AMOUNT') {
    ...
    const newBal = setBalance(userId, amount);
    adminStates[chatId] = null;
    ...
    try {
      ...
      for (const pNumber of mkPending) {
        ...
        if (record.status === 'success' && record.otps) {
          ...
          const updatedBalance = addBalance(reqData.chatId, pointsAwarded);
